...
internal class ApiRepositoryImpl @Inject constructor(private val api: ApiService) : BaseRepository,
    ApiRepository {
    override fun getLatest(): Flow<DataResource<List<Latest>>> =
        safeApiCall { api.getLatest().latests }

    override fun getSale(): Flow<DataResource<List<FlashSale>>> =
        safeApiCall { api.getSale().sales }

    override fun getDetails(): Flow<DataResource<Details>> = safeApiCall { api.getDetails() }
    override fun search(string: String): Flow<DataResource<List<String>>> =
        safeApiCall { api.search().words }
}